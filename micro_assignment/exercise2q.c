...
void add_nbr(node_t *node1, node_t *node2)
{
...
#define ITERATE_NBRS_BEGIN(node, nbrnode)                       \
  {                                                             \
                                                                \
    node_t *parent = nbrnode;                                   \
    nbrnode = NULL;                                             \
    for (int i = 0; i < MAX_NBRS && node->nbrs[i] != NULL; i++) \
    {                                                           \
      if (parent == node->nbrs[i])                              \
        continue;                                               \
      nbrnode = node->nbrs[i];
...
#define ITERATE_ONE_HOP_NBRS_BEGIN(node, nbrnode)               \
  {                                                             \
                                                                \
    for (int j = 0; j < MAX_NBRS && node->nbrs[j] != NULL; j++) \
    {                                                           \
      nbrnode = node;                                           \
      ITERATE_NBRS_BEGIN(node->nbrs[j], nbrnode);
