...
void *list_iterator(void *list_node)
{
    ...
    if (!list_node)
        return NULL;
    dll_node_t *currNode = (dll_node_t *)list_node;
    return currNode->right;
}

/*Step 3 : implement student comparator function here*/

void printStudentData(student_t *s)
{
    printf("%d\n", s->height);
    printf("%d\n", s->total_marks);
    printf("%d\n", s->weight);
...
{
    student_t *s1 = (student_t *)_studentobj1;
    student_t *s2 = (student_t *)_studentobj2;
    int same = 1;
    same &= s1->height == s2->height;
    same &= strcmp(s1->name, s2->name) == 0;
    same &= s1->total_marks == s2->total_marks;
    same &= s1->weight == s2->weight;
    same &= s1->year_of_birth == s2->year_of_birth;
    return same ? 0 : -1;
}
